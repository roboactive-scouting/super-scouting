...
- **An install with no active event locks every screen, and says the wrong thing.** **[RAISED BY ME]** *(found 2026-09-24, first production sign-in; **scheduled as `IMPLEMENTATION-PLAN.md` task 1.17b, which runs before 1.18**)*. Production has no season or event: nothing creates one before phase 1C, and the dev seed never runs there. The client then falls back to the seed event id (`App.tsx`, `FALLBACK_EVENT_ID`), the first pull fails, and the shell shows "This device has not loaded the competition yet — an internet connection is required once" **forever**, while online. Two defects:
  1. **The message is wrong.** The device is online; the real state is "no competition is set up yet". A lead reading it at a venue would go hunting for Wi-Fi that will not help.
  2. **The gate also blocks screens that need no event.** The Users screen sits behind the same hydration gate, so on an empty install the admin cannot manage accounts or create the second admin, and after phase 1C cannot reach the season/event setup that would clear the gate.
  3. **Every start shows the loading screen, even on a device that already holds the event.** The shell starts in "loading" and waits for a network sync. Leaving Change password or Login remounts it, so the loading screen reappears after every password change, and on every cold start over a slow venue connection.
  4. **Sign-in says "connect to the internet" when the server is merely unreachable.** An online browser that cannot reach the server, for example a per-deployment preview URL that CORS refuses, falls back to offline sign-in and shows "This device has not loaded the team's accounts yet. Connect to the internet once to sign in." When the device is online, it must say the server cannot be reached and suggest checking the address.

  **The fix:** tell "no active event exists" apart from "could not reach the server". With no event, show "No competition is set up yet" (an admin also gets a link to set one up, once phase 1C exists). Keep every route that does not read event data — Users, Switch scouter and, from phase 1C, season/event setup — reachable without hydration. Start from the cache: a device already hydrated for the event renders at once and syncs in the background, and the loading screen is only for a first load. Also drop the seed-event fallback from production builds, so a production install never asks the server for a dev fixture id.
...
### 5.4 Accepted risks of the auth design **[RAISED BY ME]** *(recorded 2026-09-24, phase 1B)*

Found while building phase 1B's server half (tasks 1.10–1.14). **None of items 1–4 is a bug.** Item 5 was one, and it is closed. Each is a consequence of a closed decision, written down so a later reader does not "fix" one and break the rule it protects. Change one only by reopening the decision it follows from.

1. **Any logged-in user can push operations stamped with another user's `author_user_id`, and gets that user's rights for them.** `syncPush` authorizes each operation against its own `author_user_id`, not the bearer (SPEC-FINAL §7.5); the bearer only has to be an authenticated, non-disabled user. So a scouter's device can submit an operation authored as a lead and have it treated as the lead's. **Accepted** because it is exactly the shared-tablet rule: one device, one login, several scouters' entries in one outbox, and offline switch-scouter mints no token for the person switched to. Binding the author to the bearer would make a shared tablet unable to upload anyone's work but the current user's. The mitigations are the ones already in the design: accounts are admin-provisioned, devices are team-owned, and the actor is a team member, not a stranger. **Do not** add a bearer-equals-author check to `syncPush`.
2. **Resetting a password does not sign the user out.** Tokens are stateless 30-day HS256 JWTs with no revocation list; an existing token keeps working after `resetPassword`. **Only disabling the account cuts access**, because every authenticated request re-reads the user row and refuses a disabled one. **Accepted** because a revocation store (or a per-user token generation counter checked on every request) is new state for a threat — a stolen token on a team-owned device — whose remedy already exists: disable, then re-enable or recreate. An admin responding to a lost device disables the account; a routine reset is not a security response.
3. **User administration is incomplete in phase 1B.** There is **no way to re-enable a disabled user**, **no rename** (username or full name), and **`createUser` cannot set `must_change_password`** to force a change at first login (only `resetPassword` can). **Accepted for now**: the matrix (§5.2) requires create / disable / role / reset, and those exist. The gaps are admin conveniences with workarounds (create a new account; reset immediately after create to force the change). They belong with the admin user-management screen and should be closed before the first real event, not left to be discovered then. **Scheduled as `IMPLEMENTATION-PLAN.md` task 1.17a** (added 2026-09-24).
4. **An update can resurrect an entry a lead deleted.** A soft-delete bumps the row's version; an update op whose `base_version` matches the post-delete version is applied and writes `deleted_at: null`. A lead editing a deleted row does this, and so does a scouter still inside their own five-minute window after pulling the deleted row. **Not decided here: task 1.40 (the full push semantics, SPEC-FINAL §9.5/§9.7) must decide whether an update may undelete**, or whether undelete is a distinct lead action and a plain update of a deleted row is refused. Until 1.40, the behaviour stands and is known.
5. **Production had no way to get its first admin.** It starts with no users. There is no self-registration, and `createUser` needs a signed-in admin, so merging phase 1B to `main` would have shipped an app nobody could sign in to. Of the five, this one would have stopped a season from starting. **Closed by a one-time script in the repository, `pnpm bootstrap:admin` (`packages/db/src/bootstrap/`), which a person runs by hand like a production migration** (procedure: `docs/ops/SETUP.md`, "The first admin"). It **refuses, with a non-zero exit that names who is there, if the `users` table has any row**, disabled ones included. That refusal is what makes it safe to leave in the repository forever. It creates exactly one admin with `must_change_password` set. It reads its environment only from `packages/db/.env.bootstrap`. It names the project ref it is about to write to and makes the operator type it back. The temporary password is typed twice at a prompt that does not echo, and it is never an argument, a file value or printed output. **It is the one script allowed to write to production.** Every other script guards against production; this one guards the operator. **Do not** add a production refusal to it. **Why a committed script, and not the alternatives:** hand-inserting the row in the dashboard means hand-making a bcrypt hash, and that is exactly how a plaintext password ends up pasted into a shell. A CLI with subcommands is a standing tool for a one-time act. **An HTTP bootstrap route is refused outright: an endpoint that exists at all can be called twice**, by anyone who finds it, on a public production URL. A script in the repository is reviewable because it is committed, and deliberate because a person types it at a moment they chose.
...
- **The robot is picked from a dropdown, not an open list** *(confirmed 2026-09-23, **[RAISED BY ME]** — surfaced by the phase 1A rehearsal)*. An event roster is 30–75 teams; rendering them as a scrolling list of buttons pushes everything else off a phone screen. A native select gets the platform's own picker on iOS and Android, which is faster under time pressure than a long scroll and needs no custom scroll handling. When a match and alliance are chosen the options narrow to that alliance's three robots; otherwise the full event roster is offered.
- **An option shows the team number and the team name, and nothing else.** No ids, no internal labels, no fixture prefixes. *(confirmed 2026-09-23)*
- **A robot already scouted on this device cannot be used to start a second entry** *(confirmed 2026-09-23, **[RAISED BY ME]**)*. The picker marks it as already scouted and either opens the existing entry for editing — the same behaviour §6.2's super-entry rule already gives — or, once the five-minute self-edit window (§5) has closed, says plainly that it is scouted and locked. The submit-time check stays as the backstop, but it must never be the first time a scouter hears it: discovering a duplicate *after* filling the whole form, from a message at the bottom of a review sheet, is how a scouter concludes the app is broken. **This is strictly a local-device rule.** Two devices independently entering the same (event, match, team, kind) is expected, is not blocked, and is what the duplicate path of §7 exists to reconcile.
- **Submit always says what happened.** On success the app returns to a fresh manual selection *and names what was saved* — the match and the team. On failure it stays on the entry, keeps every value, and shows the reason where the scouter is actually looking. Success wording says the entry is **saved on this device**, never that it *synced*: submitting enqueues, and the push may be minutes away or offline entirely. *(confirmed 2026-09-23, **[RAISED BY ME]** — a silent submit let three entries be believed saved when none were.)*
...
- **The unsynced count counts records, not queued operations**, so an entry for a brand-new match number reads as one unsynced, not two; the auto-created bare match rides with its entry and is never counted alone. **Tapping the indicator opens the sync page**, whose **pending list names every unsynced record** as a person recognises it (match · team, alliance, robot status, when saved, local or QR, and a rejection reason in words), with as many items as the count. *(confirmed 2026-09-23; SPEC-FINAL §9.10, v1.2)*
...
**[RAISED BY ME] 2026-09-20 — phase 1A must not reach `main` until the bearer token lands (phase 1B).**

The §19.3 vertical slice deliberately ships **no authentication**. The caller is derived from each operation’s `author_user_id`, and `GET /sync/pull` authenticates as a **`service` caller** because a pull carries no operation to derive an author from. That is correct for the slice — auth is group 1B — but it has a deployment consequence the phase plan never states.

`main` is Vercel’s **Production Branch**, and the production deployment is a **public URL backed by the production Supabase project**. Merging phase 1A into `main` would expose `POST /sync/push` and `GET /sync/pull` — unauthenticated, read *and* write — to anyone who finds the URL, against production data.

**The rule: phase 1A lands on `develop` only.** Preview is dev-only and holds no production data, so the exposure there is bounded and acceptable. The merge to `main` waits for the bearer token. Nothing needs building; this is a branch-discipline constraint, and the only cost of getting it wrong is the one that cannot be undone.

This does not weaken §19.4’s phase 1 gate: the gate is run against a preview deployment, which is exactly where phase 1A is allowed to live.
...
|---|---|---|---|
| 2026-09-24 | 4 **[RAISED BY ME]** | **Scheduled as plan task 1.17b (§4.3): the shell's hydration gate.** An install with no active event says "no competition set up yet" instead of "internet required". Users, Switch scouter and (from phase 1C) event setup stay reachable without hydration. A hydrated device starts from its cache instead of the loading screen. The seed-event fallback is dropped from production builds. **§5.4 item 3 (user-admin gaps) is scheduled as task 1.17a.** Both run before 1.18. | Found at the first production sign-in, right after the bootstrap admin changed their password: the only screen available was a wrong "internet required" message that never clears, and the admin could not reach the Users screen. The same screen reappeared after every password change, because leaving Change password remounts the shell and the shell always waited for the network. Written into the plan rather than parked in the spec, because build chats execute the plan: an item that lives only in the spec is never built. 1.17b must precede 1.18, because phase 1C's setup screens would otherwise sit behind the same gate they exist to clear. 1.17a lands in phase 1, because the spec asks for it before the first real event. |
| 2026-09-24 | 5 **[RAISED BY ME]** | **Production's first admin comes from a one-time script, `pnpm bootstrap:admin`, that a person runs by hand** (§5.4 item 5). It refuses if any user exists, creates one admin with `must_change_password` set, makes the operator type back the target project ref, and reads the temporary password from a prompt that does not echo. It is the only script allowed to write to production. | Without it, phase 1B would have locked production out of itself: no users, no self-registration, and `createUser` needs an admin. A committed script is reviewable and is run at a moment a person chooses. It also keeps bcrypt hashing in code, rather than hand-crafting a hash in the dashboard, which is how a plaintext password ends up in shell history. A CLI with subcommands is a standing tool for a one-time act. An HTTP bootstrap route was rejected because an endpoint that exists can be called twice, on a public URL. The refuse-if-any-user guard, not a production guard, is what makes it safe to keep forever. A prompt that does not echo beat a generated password, which would sit in terminal scrollback, and an environment value, which would sit in a file on disk. The password is temporary either way. |
| 2026-09-24 | 5 **[RAISED BY ME]** | **Four consequences of the auth design recorded as accepted risks in §5.4:** a bearer can push operations authored as another user and get that user's rights for them; a password reset does not end existing sessions (only disabling does); user admin lacks re-enable, rename and create-time `must_change_password`; and an update can undelete a lead-deleted entry, with task 1.40 owning that decision. | All four surfaced while building phase 1B's server half, and each follows from a decision already closed rather than from a defect. The first is the shared-tablet rule itself (SPEC-FINAL §7.5): authorizing per operation against `author_user_id` is what lets one logged-in tablet upload several scouters' work, and the only way to close it — binding author to bearer — breaks that. The second is the price of stateless tokens; a revocation store is new per-request state for a threat that disabling already answers. The third is scope, not design: §5.2's matrix is met, and the missing conveniences have workarounds. The fourth is genuinely undecided and is parked on the task that owns push semantics. Writing them down as accepted is what stops a future chat from "fixing" the first one and silently breaking shared devices. |
| 2026-09-23 | 6 **[RAISED BY ME]** | **Four data-entry rules fixed in §6.2:** the robot is picked from a **dropdown**, an option shows **team number and name only**, a robot **already scouted on this device** cannot start a second entry, and **submit always reports success or failure** — returning to a fresh selection and naming what was saved, or keeping the entry and showing why. | All four came out of the phase 1A rehearsal on a real phone, and three of them are the same failure wearing different clothes: the app did work the scouter could not see. Submit navigated nowhere and said nothing, so three entries were believed saved when validation had rejected all three; the duplicate guard fired only at submit, after the whole form was filled, from the bottom of a review sheet; and a 30-button roster buried the rest of the screen. §6.1 already says this screen decides the data quality of everything downstream, and none of these is a matter of taste — each one produced a wrong belief about whether data existed. The local-device scope on the duplicate rule is deliberate: cross-device duplicates are expected and are the conflict system's job, so blocking them here would break reconciliation to fix a UI problem. The success wording distinguishes **saved on this device** from **synced** because conflating them is the one lie an offline-first app must never tell. |
| 2026-09-23 | 7 / 16 (amendment) | **The unsynced count counts records, not operations; the indicator opens the sync page; the sync page's pending list names each record.** SPEC-FINAL §9.10 amended (v1.2). The bare match of §6.4 is never counted or listed on its own. The wipe guard keeps counting every unacked operation. | Found in the phase 1A rehearsal: one entry for a new match number showed **2 unsynced**, because Start entry queues the bare match and Submit queues the entry. The count was correct about the outbox and wrong about what a scout thinks they did. A scout who sees two for one entry either distrusts the indicator or goes looking for a second entry that doesn't exist, and the indicator is only worth anything if it is believed. Counting records also keeps the number stable when later record types (pick lists) join the outbox. The pending list was already required ("what is pending"), but not what an item shows or how to reach it; a count with no list one tap away is the same passive warning §16's Obsidian reference rules out. The wipe guard stays on raw operations because its job is never destroying anything unacked, including a match with no entry yet. |
| 2026-09-20 | 19 / 5 / 18 **[RAISED BY ME]** | **Phase 1A is not merged to `main` until the bearer token lands (phase 1B); it lives on `develop` only.** Recorded as a constraint in §19.4. | Phase 1A ships no authentication by design — the caller comes from `author_user_id`, and `GET /sync/pull` authenticates as a `service` caller because a pull carries no operation to derive an author from. Correct for the slice, but `main` is Vercel’s Production Branch and the production deployment is a public URL backed by the **production** Supabase project, so merging would expose push and pull, unauthenticated and writable, against production data. Surfaced while verifying tasks 1.1–1.4; the phase plan states the no-auth decision and the deployment topology separately and never puts them together. Preview is dev-only, so `develop` carries the slice safely and the §19.4 phase 1 gate — run against a preview deployment — is unaffected. The fix costs nothing today and the failure it prevents is unrecoverable, which is the whole argument for writing it down before it is load-bearing. |
...
|---|---|---|---|
| v0.41 | 2026-09-24 | header, 4.3 (new), 5.4, 21, 23 · IMPLEMENTATION-PLAN tasks 1.17a, 1.17b (new) | **§4.3 added: the shell's hydration gate, scheduled as plan task 1.17b.** Three defects: an empty install shows a wrong "internet required" message forever, Users is blocked, and every start (including after a password change) shows the loading screen even on a hydrated device. **§5.4 item 3's user-admin gaps are scheduled as task 1.17a.** Both run before 1.18. |
| v0.40 | 2026-09-24 | header, 5.4, 21, 23 | **§5.4 item 5 added: production had no way to get its first admin** (no users, no self-registration, `createUser` needs an admin). Closed by the one-time, hand-run `pnpm bootstrap:admin` script, which refuses if any user exists. It is the one script allowed to write to production. It was chosen over a dashboard insert, a CLI and an HTTP route, with the reasons recorded. `docs/ops/SETUP.md` gains the procedure and `BUILD-CONTEXT.md` §8 the exception. |
| v0.39 | 2026-09-24 | 5.4 (new), 21, 23 | **§5.4 added: four accepted risks of the auth design**, raised by phase 1B's server build — cross-author push on a shared bearer (the cost of the shared-tablet rule), password reset not revoking live tokens, incomplete user administration (no re-enable, no rename, no create-time `must_change_password`), and update-resurrects-a-deleted-entry, parked on task 1.40. Recorded as accepted with reasoning so none is "fixed" against the rule it protects. |
| v0.38 | 2026-09-23 | 6.2, 21, 23 | **Four data-entry rules added to §6.2**, all raised by the phase 1A airplane-mode rehearsal on a real phone: the robot picker is a **dropdown** (30–75 teams do not belong in a scrolling button list); an option carries **team number and name only**; a robot **already scouted on this device** cannot start a second entry, and is either opened for editing or reported as locked — with the rule scoped explicitly to the local device, since cross-device duplicates belong to the conflict path; and **submit always reports its outcome**, returning to a fresh selection and naming what was saved, with success worded as *saved on this device* rather than *synced*. The last of these was already implied by §6.2's existing submit bullet but was never stated as a feedback requirement, and its absence let three entries be believed saved when all three had been rejected. |
| v0.38 | 2026-09-23 | header, 16 (feedback states), 21, 23 · SPEC-FINAL header, 9.10 · IMPLEMENTATION-PLAN task 1.43 | **The unsynced count counts records, not queued operations; the indicator opens the sync page; the pending list is specified.** SPEC-FINAL bumped to v1.2. Client indicator changed to match (`unsyncedCount` in `apps/client/src/data/outbox.ts`). Task 1.43's interface now names the pending-list contents and the indicator link. |
| v0.37 | 2026-09-20 | 19.4, 21, 23 | **[RAISED BY ME] Phase 1A is barred from `main` until the bearer token lands.** The vertical slice has no authentication by design, and `main` deploys to a public production URL backed by the production Supabase project — merging it would leave `POST /sync/push` and `GET /sync/pull` open to read and write against production data. §19.4 now carries the constraint; the slice lands on `develop` only, where preview is dev-backed. No code change, no effect on the §19.4 phase 1 gate, which runs against a preview deployment. Surfaced while verifying `IMPLEMENTATION-PLAN.md` tasks 1.1–1.4. |
