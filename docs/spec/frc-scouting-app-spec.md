...
Found while building phase 1B's server half (tasks 1.10–1.14). **None of these is a bug.** Each is a consequence of a closed decision, written down so a later reader does not "fix" one and break the rule it protects. Change one only by reopening the decision it follows from.
